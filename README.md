...
**Ancile** — защищённый control plane для обмена файлами и артефактами между корпоративными AI-агентами.

Название **Ancile** взято от древнеримского священного щита. В проекте это символ защитного слоя, который охраняет обмен файлами между AI-агентами и не даёт опасным артефактам пройти дальше по workflow.
...
**Ancile** решает эту проблему как промежуточный слой между AI-агентами и корпоративными данными.
...
- Redis как инфраструктурный слой для worker, будущих очередей и realtime events;
- synthetic worker для автоматической генерации файлов;
- AES-256-GCM шифрование файлов;
- per-file DEK;
- Agent Identity через `X-Agent-Key`;
- demo mode для удобной защиты проекта;
- Policy Engine;
...
- WebSocket realtime events для backend-событий;
...
Cache / Queue:    Redis
Worker:           Synthetic Data Agent Worker
...
Worker     — 24/7 synthetic-data-agent
```
...
В проекте реализован processing flow:
...
Производные файлы получают понятные metadata:
...
## 18. Synthetic Data Agent Worker

В проекте есть отдельный worker:

```text
swf-synthetic-worker
```

Он может работать постоянно и периодически генерировать synthetic datasets.
...
1. генерирует CSV;
2. загружает файл от имени synthetic-data-agent;
3. файл шифруется;
4. запускается security scan;
5. если файл clean — запускается processing flow;
6. audit фиксирует действия.

Synthetic-файлы получают metadata:
...
- worker работает в отдельном контейнере, поэтому для production worker events должны идти через event bus.
...
Для MVP можно использовать fallback auto-refresh, чтобы новые файлы от worker появлялись без ручной кнопки обновления.

---

## 20. Enterprise Integration Model

Компании смогут использовать Ancile как security/control layer между своими AI-агентами и корпоративными данными.
...
## 21. Scalability and Large File Strategy
...
## 22. Неудобные enterprise-вопросы и roadmap
...
После `Демо → Сбросить демо` система выдаёт seed-агентам стабильные demo API keys.
...
В базе хранится не сам ключ, а только:
...
В Swagger в поле `X-Agent-Key` нужно вводить только значение ключа:
...
```text
Демо → Сбросить демо
Демо → Запустить clean scenario
Демо → Запустить risk scenario
```

После reset можно снова запустить worker:
...
Clean scenario создаёт безопасный CSV-файл и запускает processing flow.
...
Risk scenario создаёт файл с секретом и prompt injection:
...
Security Scanner находит:

- secret;
- prompt injection.

Файл получает статус:

```text
quarantined
```

Policy Engine запрещает чтение такого файла.

---

## 29. Основной сценарий демонстрации

1. Открыть Панель.
2. Показать Агенты.
3. Показать Файлы.
4. Открыть File Passport clean-файла.
5. Показать AES-GCM encryption.
6. Показать Access Graph.
7. Показать временные Permissions.
8. Запустить Policy Simulator для approved file.
9. Запустить Policy Simulator для quarantined file.
10. Показать Security / DLP findings.
11. Показать AuditLog.
12. Показать Compliance Report.
13. Показать `GET /api/auth/agent/me` с `X-Agent-Key: swf_demo_qa_key`.
14. Показать `GET /api/files/{file_id}/read-authenticated`, где backend сам определяет агента по ключу.
15. Показать MinIO bucket с encrypted objects.
16. Показать `GET /api/ready`.
17. Показать WebSocket realtime indicator.
18. Показать `docker compose ps`, где работает synthetic-worker.

---

## 30. Почему это полезно компаниям

Ancile отвечает на вопросы корпоративного использования AI agents:
...
Ancile — это защищённый control plane для обмена артефактами между корпоративными AI-агентами.
