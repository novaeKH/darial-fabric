...
export default function BudgetView() {
  ...
  const potentialOverspend = useMemo(
    () => Math.max(n(totals.forecast_cost) - n(totals.limit_amount), 0),
    [totals]
  );
      ...
      <header className="budget-hero">
        ...
        <div className="budget-actions">
          <button type="button" onClick={createAlerts} disabled={alerting}>
            ...
            {alerting ? "Расчёт…" : "Пересчитать alerts"}
          </button>
          ...
          return (
            <article className="budget-card" key={product.product_id}>
              ...
              <div className="budget-values">
                ...
                <div><span>Прогноз</span><strong>{formatMoney(product.forecast_cost)}</strong></div>
