...
Этот файл — короткий сценарий демонстрации проекта на защите.

README объясняет проект подробно, а этот документ нужен как практическая шпаргалка: что открыть, в каком порядке нажимать и что говорить.

---

## 1. Короткое объяснение проекта

**Secure Workspace Fabric** — это не обычный Dropbox для людей.

Это защищённый **control plane для обмена файлами между корпоративными ИИ-агентами**.

Агенты не должны просто передавать файлы друг другу напрямую. В корпоративной среде важно понимать:
...
Проект решает эту задачу через:
...
- File Passport;
- Access Graph;
- 24/7 Synthetic Data Agent Worker.
...
Перед reset лучше временно остановить worker, чтобы он не обращался к базе во время очистки:

```bash
docker compose stop synthetic-worker
```

Во frontend открыть вкладку **Demo** и нажать:
...
После этого снова запустить worker:

```bash
docker compose start synthetic-worker
```

Если нужно быстро обновить данные во frontend, нажать кнопку:
...
## 4. Главная речь на 1 минуту

Можно сказать так:

> Мой проект называется Secure Workspace Fabric. Это защищённая файловая шина для взаимодействия ИИ-агентов. Идея в том, что в компании агенты не должны просто обмениваться файлами напрямую. Каждый файл должен иметь владельца, уровень секретности, доступы, срок действия прав, историю использования, происхождение и security status. В проекте агенты создают файлы, система шифрует их, проверяет доступы, запускает security scan, помещает опасные файлы в quarantine, ведёт audit log и показывает lineage — из какого файла были получены другие артефакты.

---

## 5. Что показать в первую очередь
...
> На Dashboard видно текущее состояние агентской системы: сколько создано файлов, сколько агентов участвует, сколько событий попало в audit, сколько security findings найдено и сколько flow было запущено.
...
> Каждый агент является отдельным субъектом безопасности. У него есть собственный ID, роль, risk level, autonomy level, clearance level и status. Это важно, потому что действия разных агентов не смешиваются между собой.
...
> Файлы загружаются от имени конкретного агента. Каждый файл получает owner, classification, status и хранится в зашифрованном виде.
...
> File Passport — это карточка файла, где собрана вся информация о нём. Здесь видно, кто владелец файла, где он хранится, какой у него status и classification, какие есть permissions, есть ли security findings, откуда файл произошёл и какие события audit с ним связаны.
...
> Файл не хранится в открытом виде. При загрузке создаётся отдельный ключ DEK для файла, файл шифруется через AES-GCM, а зашифрованный объект хранится в MinIO. Перед чтением система сначала проверяет доступ через Policy Engine и только потом расшифровывает файл.
...
Показать интерактивный граф.

Обратить внимание на связи:
...
> На графе видно, как связаны команды, агенты, workspace, папки, файлы, permissions и lineage. Это помогает не просто хранить файлы, а видеть всю структуру доступа и происхождения артефактов.
...
> Можно поэтапно включать структуру, доступы и lineage. А режим Risky only помогает быстро найти опасные или quarantined-файлы.
...
Показать список active permissions.

Что сказать:

> Доступ можно выдать не только к файлу, но и к папке. Также можно выбрать действие и срок действия доступа. Это закрывает требование временных доступов и разграничения доступа на уровне файлов и папок.

Можно показать действие:

```text
Subject agent: qa-agent
Resource type: file или folder
Action: read
...
> Все выдачи и отзывы доступа попадают в audit log.
...
> Здесь видно, что доступ разрешён, потому что у агента есть owner access, direct file permission или доступ через папку.
...
> Даже если агенту выдан permission, quarantined-файл всё равно нельзя прочитать. Security status сильнее обычного доступа. Это важно для защиты от опасных файлов.
...
> Security-agent проверяет файл и находит секрет или prompt injection. После этого файл автоматически переводится в quarantine, и Policy Engine запрещает его чтение.
...
> Все действия агентов логируются. Можно увидеть, какой агент выполнил действие, над каким ресурсом, когда, с каким статусом и причиной.
...
> Compliance report нужен для компании или security-команды. Он показывает состояние безопасности системы: сколько файлов создано, сколько операций шифрования и расшифровки было выполнено, сколько доступов было запрещено и сколько файлов попало в quarantine.
...
> В MinIO хранятся не исходные файлы, а encrypted blobs. Метаданные лежат в PostgreSQL, а содержимое файла хранится отдельно и в зашифрованном виде.
...
> Synthetic Data Agent Worker работает отдельно и может генерировать файлы 24/7. Он создаёт synthetic datasets, загружает их от имени synthetic-data-agent, запускает security scan и processing flow.

---

## 19. Clean scenario — что происходит внутри
...
## 20. Risk scenario — что происходит внутри
...
## 21. Что делать, если reset выдаёт ошибку
...
> В итоге проект показывает не просто файловое хранилище, а инфраструктуру доверия для ИИ-агентов. Каждый агент имеет отдельную identity, каждый файл шифруется, доступы проверяются до расшифровки, опасные файлы блокируются через quarantine, все действия логируются, а lineage показывает происхождение данных. Это делает систему полезной для компаний, которые хотят использовать AI agents, но при этом контролировать безопасность, доступы и жизненный цикл файлов.

---

## 23. Короткий финальный тезис

**Secure Workspace Fabric = безопасный control plane для обмена артефактами между корпоративными ИИ-агентами.**
